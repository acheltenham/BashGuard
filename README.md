...
- `boundary` reports containment evidence for the current environment. The first slice reports when no supported backend was detected, labels that result `unknown`, explains that Pi tools inherit full user permissions only when no outer boundary exists, and states that BashGuard cannot characterize an outer container or VM from inside. It does not inspect sandbox configuration, prove that no containment exists, or describe a historical session.
- `attach` follows an active session or renders a completed session timeline. In a supported active TTY it prints the header, bounded history, and guidance first, then maintains an evidence-grounded sticky footer instead of the old startup status block. Accepted events update it immediately and freshness updates about once a second. At 72+ columns it has three content lines, at 40–71 up to four, and below 40 one compact line; truncation follows grapheme boundaries and measured terminal display cells. Startup history defaults to the latest 50 narrated events; use `--history N`, `--history 0`, or `--all-history`. Every newly appended narrated event is still displayed. Completed sessions, non-TTY or redirected output, missing or `dumb` `TERM`, and `--no-live-footer` retain ordinary plain output; these paths emit no footer-generated ANSI or cursor-control sequences. This does not guarantee that arbitrary recorded timeline payloads are ANSI-free. Raw events and complete JSONL remain available through `inspect`. Risk notices are explicitly non-blocking. See [Live attach history and status](docs/cli/live-attach.md).
...
BashGuard also does not *implement* containment. Filesystem, network, and process isolation belong to a sandbox backend. BashGuard now has a narrow adapter contract and a current-environment `boundary` report for the no-supported-backend-detected case; backend configuration detection, session-time boundary evidence, and decision observation remain planned. See [Decision 005](docs/adr/decision-log.md) and [issue #79](https://github.com/acheltenham/BashGuard/issues/79).
...
BASHGUARD_DATA_DIR=/tmp/bashguard-test node --experimental-strip-types src/cli.ts attach 1 --no-live-footer
