...
On resize BashGuard clears its tracked footer lines and redraws at the new width. Before printing a new narrated event, it clears the footer, writes the timeline event, and redraws the footer beneath it. Recorded shutdown stops refresh, clears the temporary footer, and prints one final ordinary completed status block. `Ctrl+C` and synchronous/unaccepted unexpected errors clear the temporary region and scoped handlers. If any write is accepted and later fails, BashGuard disables the footer and removes scoped handlers/listeners without cursor-up cleanup because stream, cursor, and visible-footer state are unknowable; for an ordinary error it attempts only a safe newline/plain fallback if output is subsequently writable. Accepted `EPIPE` additionally stops output quietly without another write or stack trace. Failure metadata records whether a narrated payload was accepted: clear or synchronous payload failure replays it once in plain mode, while payload-callback or redraw failure does not risk a duplicate.
