...
| Containment backend | Detect and describe an active sandbox boundary | Report what is enforced versus only observed | Partial | Slice 1 defines `SandboxAdapter` and ships `bashguard boundary` with `NoSandboxAdapter` for the no-supported-backend-detected case; the result is `unknown`, not proof of absence. Pi ships no built-in sandbox by design. A first-party example extension wraps `@anthropic-ai/sandbox-runtime` (`sandbox-exec` on macOS, `bubblewrap` on Linux) and mediates `bash` and user `!` commands only — not `read`, `write`, or `edit`. Configuration detection, active-backend observation, and session-time evidence remain unimplemented |
