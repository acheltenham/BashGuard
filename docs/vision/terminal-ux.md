...
Completed sessions, non-TTY output, pipes/redirects, missing `TERM`, `TERM=dumb`, and `--no-live-footer` keep the ordinary static status and timeline behavior and emit no footer-generated ANSI or cursor-control sequences. Arbitrary recorded timeline payloads are not universally guaranteed to be ANSI-free. The footer uses neither an alternate screen nor a hidden cursor and is not the planned full split-pane TUI. Users may refine the labels and prioritization after using it in real sessions.
